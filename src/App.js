...
• Built strategic partnerships and maintained relationships with key clients and vendors`;
  ...
  function copyToClipboard() {
    ...
    navigator.clipboard.writeText(text)
      ...
      .catch(() => showStatus('Failed to copy to clipboard. Please select and copy manually.', 'error'));
  }
  function downloadResume() {
    ...
    let content;
    if (currentFormat === 'json') {
      content = JSON.stringify(optimizedResumeData, null, 2);
    } else if (currentFormat === 'html') {
      content = formatAsHTML(optimizedResumeData.content);
    } else {
      content = optimizedResumeData.content;
    }
    const extension = currentFormat === 'json' ? 'json' : (currentFormat === 'html' ? 'html' : 'txt');
  ...
  function printResume() {
    ...
    win.document.write(`<!DOCTYPE html><html><head><title>Optimized Resume</title><style>body{font-family:Georgia,serif;line-height:1.6;margin:0.5in;}h1,h2{color:#1e293b;}h2{border-bottom:1px solid #ccc;padding-bottom:5px;}ul{margin:0;padding-left:20px;}li{margin-bottom:5px;}</style></head><body>${currentFormat==='html'?formatAsHTML(optimizedResumeData.content):`<pre>${currentFormat==='json'?JSON.stringify(optimizedResumeData,null,2):optimizedResumeData.content}</pre>`}</body></html>`);
    win.document.close();
    win.print();
  }
  function emailResume() {
    ...
    let body;
    if (currentFormat === 'json') {
      body = encodeURIComponent(JSON.stringify(optimizedResumeData, null, 2));
    } else if (currentFormat === 'html') {
      body = encodeURIComponent(formatAsHTML(optimizedResumeData.content));
    } else {
      body = encodeURIComponent(optimizedResumeData.content);
    }
...
• Implement best practices for security and performance`;
  ...
  function handleSetFormat(format) {
